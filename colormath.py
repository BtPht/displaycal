...
def get_whitepoint(whitepoint=None, scale=1.0, planckian=False):
    ...
    if isinstance(whitepoint, str):
        ...
        if planckian:
            ...
            if not whitepoint:
                raise ValueError(
                    "Planckian color temperature %i out of range " "(1667, 25000)" % cct
                )
        else:
            whitepoint = CIEDCCT2XYZ(cct)
            if not whitepoint:
                raise ValueError(
                    "Daylight color temperature %i out of range " "(2500, 25000)" % cct
                )
...
def test():
    for i in range(4):
        ...
        print(
            (
                "RGB and corresponding XYZ (nominal range 0.0 - 1.0) with "
                "whitepoint %s" % wp
            )
        )
