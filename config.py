...
def get_argyll_display_number(geometry):
    ...
    for i, display in enumerate(getcfg("displays")):
        if display.find("@ " + geometry) > -1:
            if debug:
                safe_print("[D] Found display %s at index %i" % (geometry, i))
            return i
...
def get_display_profile(display_no=None):
    ...
    try:
        ...
        _safe_print(
            "ICCP.get_display_profile(%s):" % display_no,
            ...
            if os.path.isfile(cfgfile):
                try:
                    ...
                    safe_print(
                        ...
                        % tuple(safe_unicode(s) for s in (cfgfile, exception))
                    )
    ...
    if which == "user":
        ...
        try:
            ...
            if getcfg("argyll.dir"):
                cfgfile.write(
                    os.linesep.join(
                        ["[Default]", "%s = %s" % ("argyll.dir", getcfg("argyll.dir"))]
                    )
                    + os.linesep
                )
            cfgfile.close()
