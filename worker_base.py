...
def check_argyll_bin(paths=None):
    ...
    for name in argyll_names:
        ...
        if prev_dir:
            ...
            prev_dir = cur_dir
    if verbose >= 3:
        safe_print("Argyll binary directory:", cur_dir)
    if debug:
        safe_print("[D] check_argyll_bin OK")
    if debug >= 2:
        if not paths:
    ...
    def create_tempdir(self):
        ...
        if not self.tempdir or not os.path.isdir(self.tempdir):
            ...
            if verbose >= 2:
                ...
                safe_print(
                    appname + ": Creating a new temporary directory " "because", msg
                )
            try:
        ...
        if self.temp and os.path.isfile(self.profile_path):
            ...
            if self.tempdir and not os.listdir(self.tempdir):
                try:
                    ...
                    safe_print(
                        "Warning - temporary directory '%s' could "
                        "not be removed: %s"
                        % tuple(safe_unicode(s) for s in (self.tempdir, exception))
                    )
