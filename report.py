...
def update(report_path, pack=True):
    ...
    data = (
        ...
        ("${CORRECTION_MATRIX}", '"FF_correction_matrix"\s*value="(.+?)"\s*/?>', 0),
        ("${BLACKPOINT}", '"FF_blackpoint"\s*value="(.+?)"\s*/?>', 0),
        ("${WHITEPOINT}", '"FF_whitepoint"\s*value="(.+?)"\s*/?>', 0),
        (
            "${WHITEPOINT_NORMALIZED}",
            '"FF_whitepoint_normalized"\s*value="(.+?)"\s*/?>',
            0,
        ),
        ("${PROFILE}", '"FF_profile"\s*value="(.+?)"\s*/?>', 0),
        ("${PROFILE_WHITEPOINT}", '"FF_profile_whitepoint"\s*value="(.+?)"\s*/?>', 0),
        (
            "${PROFILE_WHITEPOINT_NORMALIZED}",
            '"FF_profile_whitepoint_normalized"\s*value="(.+?)"\s*/?>',
            0,
        ),
        ...
        ("${TRC}", 'TRC\s*=\s*"(.+?)"[;,]', 0),
        ("${WHITEPOINT_SIMULATION}", "WHITEPOINT_SIMULATION\s*=\s*(.+?)[;,]", 0),
        (
            "${WHITEPOINT_SIMULATION_RELATIVE}",
            "WHITEPOINT_SIMULATION_RELATIVE\s*=\s*(.+?)[;,]",
            0,
        ),
