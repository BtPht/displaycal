    ...
    if forward_xicclu and backward_xicclu:
        ...
        if Cmode == "primaries_secondaries":
            for i in range(6):
                ...
                safe_print(
                    "Display XYZ %5.2f %5.2f %5.2f" % tuple(v * 100 for v in XYZdisp)
                )
            ...
            if device_id:
                try:
                    ...
                    if profile_path:
                        ...
                        if path_only:
                            safe_print(
                                "Got profile from colord for display %i "
        ...
        for handle in handles:
            ...
            if handle_name and handle_name.endswith(substr):
                ...
                try:
                    ...
                    safe_print(
                        "Couldn't close handle 0x%x:" % handle.HandleValue, exception
                    )
        ...
        for option in options:
            if sys.platform == "darwin":
                ...
                if (
                    ...
                    and not None in (x_hostname, x_display, x_screen)
                ):
...
def hexrepr(bytestring, mapping=None):
    ...
    ascii = safe_unicode(re.sub("[^\x20-\x7e]", "", bytestring)).encode(
        "ASCII", "replace"
    )
    if ascii == bytestring:
        hexrepr += " '%s'" % ascii
            ...
            for a in range(clutres):
                for b in range(clutres):
                    for c in range(clutres):
                        ...
                        stream.write(
                            "%i %7.3f %7.3f %7.3f %10.6f %10.6f %10.6f\n"
                            % (i, R, G, B, X, Y, Z)
                        )
                        i += 1
                    block += 1
...
class ChromaticityType(ICCProfileTag, Colorant):
    def __init__(self, tagData=None, tagSignature=None):
        ...
        channels = tagData[12:]
        for count in range(deviceChannelsCount):
...
class ColorantTableType(ICCProfileTag, AODict):
    def __init__(self, tagData=None, tagSignature=None, pcs=None):
        ...
        data = tagData[12:]
        for count in range(colorantCount):
            pcsvalues = [
            ...
            for i, pcsvalue in enumerate(pcsvalues):
                if pcs in ("Lab", "RGB", "CMYK", "YCbr"):
                    ...
                    safe_print(
                        "Warning: Non-standard profile connection " "space '%s'" % pcs
                    )
                    return
            end = data[:32].find("\0")
            if end < 0:
...
class CurveType(ICCProfileTag, list):
    def __init__(self, tagData=None, tagSignature=None, profile=None):
        ...
        if curveEntriesCount == 1:
            ...
            # Curve
            for count in range(curveEntriesCount):
...
class DictType(ICCProfileTag, AODict):
    ...
    def __init__(self, tagData=None, tagSignature=None):
        ...
        if recordlen not in (16, 24, 32):
            safe_print(
                "Error (non-critical): '%s' invalid record length "
                "(expected 16, 24 or 32, got %s)" % (tagData[:4], recordlen)
            )
            return
        elements = {}
        for n in range(0, numrecords):
            ...
            if len(record) < recordlen:
                safe_print(
                    "Error (non-critical): '%s' record %s too short "
...
class MultiLocalizedUnicodeType(ICCProfileTag, AODict):  # ICC v4
    def __init__(self, tagData=None, tagSignature=None):
        ...
        if recordSize != 12:
            safe_print(
                "Warning (non-critical): '%s' invalid record length "
            ...
            while count:
                ...
                for desc_type in ("dmnd", "dmdd"):
                    ...
                    if tag_type == "desc":
                        ...
                        safe_print(
                            "Error (non-critical): could not fully "
            ...
            try:
                if charBytes == 1:
                    ...
                    if unicodeDescription[:2] == "\xfe\xff":
                        # UTF-16 Big Endian
                        if debug:
                            safe_print("UTF-16 Big endian")
                        unicodeDescription = unicodeDescription[2:]
                        if (
                            len(unicodeDescription.split(" "))
                        ...
                        if debug:
                            safe_print("UTF-16 Little endian")
                        unicodeDescription = unicodeDescription[2:]
                        ...
                        if debug:
                            safe_print("ASSUMED UTF-16 Big Endian")
                        unicodeDescription = str(
                            unicodeDescription, "utf-16-be", errors="replace"
                        )
    ...
    def tagData():
        ...
        def fget(self):
            tagData = [
                ...
                safe_unicode(self.ASCII).encode("ASCII", "replace")
                + "\0",  # ASCII desc, \0 terminated
            ...
            if "Unicode" in self:
                tagData.extend(
                    [
                        ...
                        "\xfe\xff"
                        + self.Unicode.encode("utf-16-be", "replace")
                        + "\0\0",
                    ]
            ...
            if "Macintosh" in self:
                ...
                tagData.extend(
                    [
                        ...
                        + ("\0" * (67 - len(macDescription))),
                    ]
                )
            else:
                tagData.extend(["\0", "\0" * 67])  # Mac desc length = 0
            return "".join(tagData)
...
class NamedColor2Value(object):
    def __init__(
        self, valueData="\0" * 38, deviceCoordCount=0, pcs="XYZ", device="RGB"
    ):
        self._pcsname = pcs
    ...
    @property
    def name(self):
        return str(Text(self.rootName.strip("\0")), "latin-1")
...
class NamedColor2Type(ICCProfileTag, AODict):
    ...
    def __init__(self, tagData="\0" * 84, tagSignature=None, pcs=None, device=None):
        ICCProfileTag.__init__(self, tagData, tagSignature)
        AODict.__init__(self)
...
class ICCProfile(object):
    ...
    def __new__(cls, profile=None, load=True, use_cache=False):
        ...
        if isinstance(profile, str):
            if profile.find("\0") < 0 and not profile.startswith("<"):
                ...
                if (
                    not os.path.isfile(profile)
                    and not os.path.sep in profile
                    and (
                        not isinstance(os.path.altsep, str)
                        or not os.path.altsep in profile
                    )
                ):
                    for path in iccprofiles_home + [
                        x for x in iccprofiles if x not in iccprofiles_home
                    ]:
        ...
        if profile is not None:
            ...
            if data[36:40] != "acsp":
                raise ICCProfileInvalidError(
                    "Profile signature mismatch - "
                    "expected 'acsp', found '" + data[36:40] + "'"
                )
    ...
    def header(self, tagTableSize, tagDataSize):
        ...
        header.extend(
            [
                ...
                self.device["manufacturer"][:4].rjust(4, "\0")
                if self.device["manufacturer"]
                else "\0" * 4,
                self.device["model"][:4].rjust(4, "\0")
        ...
        header.extend(
            [
                uInt32Number_tohex(deviceAttributes) + "\0" * 4,
            ...
            if self._data and len(self._data) > 131:
                ...
                while tagTable:
                    ...
                    if tagSignature in self._tags:
                        safe_print(
                            "Error (non-critical): Tag '%s' already "
                        ...
                        if (tagDataOffset, tagDataSize) in tags:
                            ...
                            if len(typeSignature) < 4:
                                safe_print(
                                    "Warning: Tag type signature for "
    ...
    def from_XYZ(
        ...
        profile.device["manufacturer"] = (
            "\0\0" + manufacturer_id[1] + manufacturer_id[0]
        )
    ...
    def add_device_info(info, device, level=1):
        ...
        info[indent + "Manufacturer"] = (
            "0x%s" % binascii.hexlify(device.get("manufacturer", "")).upper()
        )
        if (
            len(device.get("manufacturer", "")) == 4
            and device["manufacturer"][0:2] == "\0\0"
            and device["manufacturer"][2:4] != "\0\0"
        ):
            ...
            manufacturer = safe_unicode(
                re.sub("[^\x20-\x7e]", "", device.get("manufacturer", ""))
    ...
    def get_info(self):
        ...
        for sig, tag in self.tags.items():
            ...
            if isinstance(tag, chromaticAdaptionTag):
                ...
                info[name] = "'%s' [%i Bytes]" % (tag.tagData[:4], len(tag.tagData))
        return info
    ...
    def set_gamut_metadata(self, gamut_volume=None, gamut_coverage=None):
        ...
        if gamut_volume or gamut_coverage:
            if not "meta" in self.tags:
                self.tags.meta = DictType()
            # Update meta prefix
            prefixes = (self.tags.meta.getvalue("prefix", "", None) or "GAMUT_").split(
                ","
            )
