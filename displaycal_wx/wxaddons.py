...
def AdjustMinMax(self, minvalue=0.0, maxvalue=1.0):
    ...
    for i, byte in enumerate(buffer):
        buffer[i] = chr(
            min(int(round(minvalue * 255 + ord(byte) * (maxvalue - minvalue))), 255)
        )
...
def GammaCorrect(self, from_gamma=-2.4, to_gamma=1.8):
    ...
    for i, byte in enumerate(buffer):
        buffer[i] = chr(
            int(
                round(
                    specialpow(ord(byte) / 255.0, from_gamma) ** (1.0 / to_gamma) * 255
                )
            )
        )
...
def GetAllChildren(self, skip=None):
    ...
    for child in children:
        ...
        if hasattr(child, "GetAllChildren") and hasattr(
            child.GetAllChildren, "__call__"
        ):
