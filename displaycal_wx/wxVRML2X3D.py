        ...
        def __init__(self, html, embed, view, force, cache):
            ...
            self.SetIcons(
                config.get_icon_bundle(
                    [256, 48, 32, 16], appname + "-VRML-to-X3D-converter"
                )
            )
...
def main():
    if "--help" in sys.argv[1:] or (not sys.argv[1:] and not gui):
        ...
        safe_print("Usage: %s [OPTION]... FILE..." % os.path.basename(sys.argv[0]))
        safe_print("The output is written to FILENAME.x3d(.html)")
        safe_print("")
        safe_print(
            "  --embed      Embed viewer components in HTML instead of "
            "referencing them"
        )
        safe_print("  --force      Force fresh download of viewer components")
        safe_print(
            "  --no-cache   Don't use viewer components cache (only "
            "uses existing cache if"
        )
        safe_print(
            "               embedding components, can be overridden " "with --force)"
        )
        if gui:
