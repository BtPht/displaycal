...
class LogFile:
    ...
    def __init__(self, filename, logdir, when="never", backupCount=0):
        ...
        self._logger = get_file_logger(
            md5(safe_str(filename, "UTF-8")).hexdigest(),
            when=when,
...
class SafeLogger(SafePrinter):
    ...
    def __init__(
        self, log=True, print_=hasattr(sys.stdout, "isatty") and sys.stdout.isatty()
    ):
...
def get_file_logger(
                        ...
                        for filename in filenames:
                            ...
                            try:
                                ...
                                safe_print(
                                    ...
                                    % tuple(
                                        safe_unicode(s) for s in (logfile, exception)
                                    )
                                )
                            else:
            ...
            if now[:3] > mtime[:3]:
                ...
                if os.path.exists(logbackup):
                    try:
                        ...
                        safe_print(
                            "Warning - logfile backup '%s' could not "
                            "be removed during rollover: %s"
                            % tuple(safe_unicode(s) for s in (logbackup, exception))
                        )
                try:
                    ...
                    safe_print(
                        ...
                        % tuple(
                            safe_unicode(s)
                            for s in (logfile, os.path.basename(logbackup), exception)
                        )
                    )
                        ...
                        for logbackup in result[: len(result) - backupCount]:
                            try:
                                ...
                                safe_print(
                                    ...
                                    "rollover: %s"
                                    % tuple(
                                        safe_unicode(s) for s in (logbackup, exception)
                                    )
                                )
    if os.path.exists(logdir):
        try:
            ...
            safe_print(
                "Warning - logging to file '%s' not possible: %s"
                % tuple(safe_unicode(s) for s in (logfile, exception))
            )
    return logger
