...
class Tag(object):
    ...
    def html(self, title="Untitled", xhtml=False, embed=False, force=False, cache=True):
        ...
        x3d_html = re.sub("\s*</?X3D(?:\s+[^>]*)?>\s*", "", self.markup(xhtml, True))
        if not xhtml:
            # Convert uppercase letters at start of tag name to lowercase
            x3d_html = re.sub(
                "(</?[0-9A-Z]+)", lambda match: match.groups()[0].lower(), x3d_html
            )
        # Indent
        x3d_html = "\n".join(
        ...
        html = re.sub(
            "(<title>)[^<]*(</title>)",
            create_replace_function(r"\1%s\2", safe_unicode(title).encode("UTF-8")),
            html,
        )
        # Insert X3D
	...
	children [
        ...
        for i, axis in enumerate(axes):
            ...
            vrml = vrml.replace(
                axis,
                re.sub(
                    "diffuseColor\s+1\.0+\s+0\.0+\s+0\.0+",
            ...
            vrml = vrml.replace(
                axis,
                re.sub(
                    "diffuseColor\s+0\.0+\s+1\.0+\s+0\.0+",
            ...
            vrml = vrml.replace(
                axis,
                re.sub(
                    "diffuseColor\s+1\.0+\s+1\.0+\s+0\.0+",
            ...
            vrml = vrml.replace(
                axis,
                re.sub(
                    "diffuseColor\s+0\.0+\s+0\.0+\s+1\.0+",
