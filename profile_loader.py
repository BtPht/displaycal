...
from .meta import VERSION, VERSION_BASE
    ...
    class ProfileAssociationsDialog(InfoDialog):
        ...
        def _update_device(self, fn, arg0, devicekey, show_error=True):
            ...
            try:
                ...
                if show_error:
                    wx.CallAfter(
                        show_result_dialog, UnloggedError(safe_str(exception)), self
                    )
        ...
        if sys.platform == "win32":
            ...
            class TaskBarIcon(SysTrayIcon):
                ...
                def _active_icon():
                    def fget(self):
                        if debug > 1:
                            safe_print("[DEBUG] _active_icon[%i]" % self._icon_index)
                ...
                def set_auto_restore(self, event):
                    safe_print(
                        "Menu command: Preserve calibration state", event.IsChecked()
                    )
        ...
        for i, display in enumerate(
            ...
            if dispwin:
                ...
                if (
                    ...
                    if errortxt and (
                        (
                            not "using linear" in errortxt
                            and not "assuming linear" in errortxt
                        )
        ...
        if sys.getwindowsversion() >= (6,):
            ...
            if os.path.basename(exe).lower() in ("python.exe", "pythonw.exe"):
                ...
                if os.path.exists(pyw):
                    ...
                    # command
                    if re.match(
                        "sha\d+(?:new)?", os.path.basename(os.path.dirname(pydir))
                    ):
                        cmd = which("0install-win.exe") or "0install-win.exe"
                        loader_args.extend(
                            [
    ...
    def _check_display_changed(self, first_run=False, dry_run=False):
        ...
        try:
            ...
            if exception.args[0] != errno.ENOENT or sys.getwindowsversion() >= (6,):
                warnings.warn(
                    r"Registry access failed: %s: HKLM\%s"
                    % (safe_str(exception), key_name),
                    Warning,
                )
            key = None
            numsubkeys = 0
        ...
        for i in range(numsubkeys):
            ...
            try:
                ...
                warnings.warn(
                    ...
                    % (safe_str(exception), value_name, key_name, subkey_name),
                    Warning,
                )
                continue
        ...
        while self and self.monitoring:
            ...
            for i, (display, edid, moninfo, device) in enumerate(self.monitors):
                ...
                try:
                    ...
                    if debug:
                        safe_print(
                            "Display %s (%s) no longer present?" % (key, display)
                        )
                    self._next = False
                    break
                except Exception as exception:
                ...
                if not vcgt_ramp:
                        ...
                        if not self.profiles.get(key):
                            try:
                                ...
                                if (
                                    ...
                                    and not "vcgt" in self.profiles[key].tags
                                ):
                    ...
                    if len(vcgt_values[0]) != 256:
                        ...
                        safe_print("Caching gamma ramps for profile", desc)
                ...
                if (
                    ...
                    safe_print("Preserving calibration state for display", display)
                try:
                    hdc = win32gui.CreateDC(moninfo["Device"], None, None)
                except Exception as exception:
                    if exception.args != self._last_exception_args or debug:
    ...
    def _enumerate_monitors(self):
        ...
        for i, moninfo in enumerate(monitors):
            ...
            if debug or verbose > 1:
                ...
                if device:
                    ...
                    safe_print(
                        "Monitor %i active display device ID:" % i, device.DeviceID
                    )
                    safe_print(
                        "Monitor %i active display device key:" % i, device.DeviceKey
                    )
                else:
                    safe_print("WARNING: Monitor %i has no active display device" % i)
            ...
            if (debug or verbose > 1) and device0:
                safe_print(
                    "Monitor %i 1st display device name:" % i, device0.DeviceName
                )
                safe_print(
                    "Monitor %i 1st display device string:" % i, device0.DeviceString
                )
                safe_print(
                ...
                safe_print("Monitor %i 1st display device key:" % i, device0.DeviceKey)
            if device0:
            ...
            if (
                ...
                and not device0.DeviceKey in self.display_devices
            ):
        ...
        for display, edid, device, device0 in self.display_devices.values():
            ...
            if not device.StateFlags & DISPLAY_DEVICE_ACTIVE:
                display_parts.append(" (%s)" % lang.getstr("deactivated"))
    ...
    def _set_display_profiles(self, dry_run=False):
        ...
        for i, (display, edid, moninfo, device) in enumerate(self.monitors):
            ...
            for device in devices:
                ...
                if debug or verbose > 1:
                    safe_print(
                        "%s (%s): %s" % (display_edid[0], device.DeviceKey, profile)
                    )
...
def get_display_name_edid(device, moninfo=None, index=None, include_adapter=False):
    ...
    if moninfo:
        ...
        if moninfo.get("_adapter") and include_adapter:
            display += " - %s" % moninfo["_adapter"].DeviceString
...
def get_profile_desc(profile_path, include_basename_if_different=True):
    ...
    try:
        ...
        if not isinstance(exception, IOError):
            exception = traceback.format_exc()
        safe_print("Could not get description of profile %s:" % profile_path, exception)
    else:
...
def main():
    ...
    if "--help" in sys.argv[1:] or unknown_option:
        ...
        safe_print(
            "  --force          Force loading of calibration/profile (if it has been"
        )
