# NOTE this is actually the Python 2.7.15 subprocess module made compatible
# with Python 2.6
...
def _args_from_interpreter_flags():
    ...
    for flag, opt in list(flag_opt_map.items()):
        ...
        if v > 0:
            args.append("-" + opt * v)
    if getattr(sys.flags, "hash_randomization", 0) != 0:
        args.append("-R")
    ...
    if mswindows:
        ...
        def _communicate(self, input):
            ...
            if self.stdout:
                ...
                stdout_thread = threading.Thread(
                    target=self._readerthread, args=(self.stdout, stdout)
                )
            ...
            if self.stderr:
                ...
                stderr_thread = threading.Thread(
                    target=self._readerthread, args=(self.stderr, stderr)
                )
        ...
        def send_signal(self, sig):
            ...
            if sig == signal.SIGTERM:
                self.terminate()
            elif sig == CTRL_C_EVENT:
                os.kill(self.pid, CTRL_C_EVENT)
            elif sig == CTRL_BREAK_EVENT:
                os.kill(self.pid, CTRL_BREAK_EVENT)
            else:
