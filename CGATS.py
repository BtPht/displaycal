...
class CGATS(dict):
        ...
        if cgats:
            ...
            for raw_line in raw_lines:
                ...
                if line == "BEGIN_DATA_FORMAT":
                    ...
                    values
                    and values[0] not in ("Comment:", "Date:")
                    and len(line) >= 3
                    and not re.search("[^ 0-9A-Za-z/.]", line)
                ):
            ...
            if 0 in self and self[0].get("NORMALIZED_TO_Y_100") == "NO":
                ...
                if self[0].normalize_to_y_100():
                    ...
                    safe_print("Warning: Could not normalize to Y = 100:", reprstr)
    ...
    def __delitem__(self, name):
        if (
            self.type not in ("DATA", "DATA_FORMAT", "KEYWORDS", "SECTION")
    ...
    def get_descriptor(self, localized=True):
        ...
        if not desc or desc == "Not specified" or is_ccss:
            if not is_ccss:
                ...
                if tech:
                    if desc and desc != "Not specified" and desc != "CCSS for " + tech:
                        display = desc
                    else:
                        display = self.queryv1("DISPLAY")
                    if localized:
    ...
    def __str__(self):
        ...
        if (
            (self.parent and self.parent.type or self.type) == "ROOT"
            and result
            and result[-1] != ""
            and lvl == 0
        ):
    ...
    def checkerboard(
        ...
        if split_grays:
                ...
                if is_gray:
                    if not prev_values:
                        ...
                        if prev_i not in added:
                            if debug:
                                safe_print(
                                    "INFO - appending prev %s to color "
                                    "because prev was same gray but got "
                                    "skipped" % prev_values[:3]
                                )
                        if debug:
                            safe_print(
                                "INFO - appending cur to gray because "
                                "prev %s was same gray" % prev_values[:3]
                            )
                        ...
                        if debug:
                            safe_print(
                                "INFO - appending cur to gray because "
                                "prev %s was different gray" % prev_values[:3]
                            )
                    elif i < numvalues - 1:
                        if debug:
                            safe_print(
                                "WARNING - skipping gray because "
                                "prev %s was not gray" % prev_values[:3]
                            )
                    else:
                        # Last
                        if debug:
                ...
                if not is_gray or cur is gray or i == numvalues - 1:
                    if prev_i not in added:
                        if debug and prev is cur is color:
                            safe_print(
                                "INFO - appending prev %s to color because "
            ...
            if (
                ...
                if debug:
                    safe_print(
                        "INFO - appending color to gray because color "
            ...
            if debug:
                for i, values in enumerate(gray):
                    safe_print(
                        "%4i" % (i + 1), "GRAY", ("%8.4f " * 3) % tuple(values[:3])
                    )
                for i, values in enumerate(color):
                    safe_print(
                        "%4i" % (i + 1), "COLOR", ("%8.4f " * 3) % tuple(values[:3])
                    )
        else:
            color = valueslist
        checkerboard = []
        ...
        if len(checkerboard) != numvalues:
            ...
            safe_print(
                "Number of patches incorrect after re-ordering (is %i, "
    ...
    def add_data(self, data, key=None):
        ...
        if self.type == "DATA":
                        ...
                        if item.upper() in ("INDEX", "SAMPLE_ID", "SAMPLEID"):
                            ...
                            try:
                                ...
                                raise CGATSValueError(
                                    ...
                                    "%s (expected float, "
                                    "got %s)" % (item, type(value))
                                )
                            else:
                                strval = str(abs(value))
                                if (
                                    self.parent.type != "CAL"
                                    and item.startswith("RGB_")
                                    or item.startswith("CMYK_")
                                ):
                            ...
                            self.root.normalize_fields and item.upper() == "SAMPLENAME"
                        ):
                            item = "SAMPLE_NAME"
                        dataset[item] = value
                    if type(key) == int:
            ...
            if isinstance(data, str) and data.find("\n") < 0 and data.find("\r") < 0:
                ...
                raise CGATSTypeError(
                    "Invalid data type for %s (expected str "
                    "or unicode without line endings, got %s)" % (self.type, type(data))
                )
                ...
                for var in data:
                    if var in ("NUMBER_OF_FIELDS", "NUMBER_OF_SETS"):
                        ...
                        if (
                            self.root.normalize_fields
                            and (
                                self.type in ("DATA_FORMAT", "KEYWORDS")
                                or var == "KEYWORD"
                            )
