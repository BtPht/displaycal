...
def cal_to_fake_profile(cal):
    ...
    profile._tags.desc.ASCII = safe_unicode(os.path.basename(cal.filename)).encode(
        "ascii", "asciize"
    )
...
def cal_to_vcgt(cal, return_cgats=False):
    ...
    if not isinstance(cal, CGATS.CGATS):
        try:
            ...
            safe_print(
                "Warning - couldn't process CGATS file '%s': %s"
    ...
    if data_format:
        for field in required_fields:
            if not field in list(data_format.values()):
                if debug:
                    safe_print("[D] Missing required field:", field)
                return None
        for field in list(data_format.values()):
            if not field in required_fields:
                if debug:
                    safe_print("[D] Unknown field:", field)
                return None
...
def extract_cal_from_profile(
    ...
    if (
        ...
        if white and (black, white) != (0, 255):
            safe_print(
                "Need to un-scale vcgt from video levels (%s..%s)" % (black, white)
            )
            # Need to un-scale video levels
            data = cgats.queryv1("DATA")
            if data:
                safe_print(
                    "Un-scaling vcgt from video levels (%s..%s)" % (black, white)
                )
                ...
                for entry in data.values():
                    for column in "RGB":
                        ...
                        if lvl < round(black, 2) or lvl > round(white, 2):
                            ...
                            safe_print(
                                "Warning: Metadata claims video levels "
                ...
                safe_print("Warning - no un-scaling applied - no " "calibration data!")
    if out_cal_path:
...
def extract_fix_copy_cal(source_filename, target_filename=None):
    ...
    if "CIED" in profile.tags or "targ" in profile.tags:
        cal_lines = []
        ti3 = StringIO(profile.tags.get("CIED", "") or profile.tags.get("targ", ""))
                    ...
                    if options_dispcal:
                        ...
                        for o in options_dispcal:
                            ...
                            if o[0] == "f":
                                ...
                                cal_lines.append(
                                    'DEGREE_OF_BLACK_OUTPUT_OFFSET "%s"' % o[1:]
                                )
                                continue
                            if o[0] == "k":
                                cal_lines.append("KEYWORD " '"BLACK_POINT_CORRECTION"')
                                cal_lines.append('BLACK_POINT_CORRECTION "%s"' % o[1:])
                                continue
                            if o[0] == "B":
                                cal_lines.append("KEYWORD " '"TARGET_BLACK_BRIGHTNESS"')
                                cal_lines.append('TARGET_BLACK_BRIGHTNESS "%s"' % o[1:])
                                continue
                            if o[0] == "q":
                                if o[1] == "l":
                                    q = "low"
